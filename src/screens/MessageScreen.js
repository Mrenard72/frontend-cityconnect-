...
export default function MessageScreen() {
  ...
  /********************************************************
   * Étape 1 : Récupérer d’abord le userId via /auth/profile
   * ******************************************************/
  useEffect(() => {
  ...
  /*********************************************************
   * Étape 2 : Charger la conversation APRES avoir userId
   * *******************************************************/
  useEffect(() => {
    // On attend que userId ET conversationId soient définis
    ...
    async function fetchConversation() {
      try {
        const token = await AsyncStorage.getItem('token');
        if (!token) {
          console.log('Pas de token, pas de fetch...');
          return;
        }
        ...
        // conversation.messages -> { _id, sender: { _id, username }, content, timestamp }
        const loaded = conversation.messages.map((m) => {
          // Compare l’ID du sender avec le userId
          const isMe = (m.sender && m.sender._id === userId);
          return {
            id: m._id,
            text: m.content,
            sender: isMe ? 'me' : 'other',
            senderName: m.sender?.username || 'Participant',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const finalMsg = {
        ...
        senderName: serverMsg.sender?.username || 'Participant',
      ...
      // Optionnel => retirer le message local
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View>
              {/* On n’affiche le pseudo que si c’est “other” */}
              {!isMe && (
      ...
      {/* Champ de saisie du nouveau message */}
